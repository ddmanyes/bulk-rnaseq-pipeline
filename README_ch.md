...
1. 在專案資料夾中找到 **`run_pipeline.command`** 檔案。
2. **雙擊 (Double-click)** 該檔案。
3. 終端機視窗將會開啟，自動安裝所需的 Python 套件，並在瀏覽器中啟動 GUI。

### Windows 使用者

1. 在專案資料夾中找到 **`run_pipeline_win.bat`** 檔案。
2. **雙擊 (Double-click)** 該檔案。
3. 命令提示字元視窗將會開啟，自動設定環境並啟動 GUI。
